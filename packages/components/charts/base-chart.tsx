...
export function ChartLegend({
  ...
  items: { name: string; color: string; strokeWidth?: number }[];
}) {
  const ctx = useBaseChart();
  const canvasRef = React.useRef<HTMLCanvasElement>(null);
  ...
  React.useEffect(() => {
    ...
    canvas.height = (items.length * 25 + 10) * dpr;

    const context = canvas.getContext("2d");
    if (!context) return;

    context.scale(dpr, dpr);
    context.clearRect(0, 0, 200, items.length * 25 + 10);
    ...
    items.forEach((item, idx) => {
      ...
      context.textBaseline = "middle";
      context.fillText(item.name, 35, y);
    });
  }, [items, ctx.devicePixelRatio]);
...
export function TimeSeriesControls() {
  const ctx = useBaseChart();
  const { timeSeriesState, setTimeSeriesState } = ctx;

  React.useEffect(() => {
    if (!timeSeriesState || !timeSeriesState.isPlaying) return;
    ...
    const interval = setInterval(() => {
      setTimeSeriesState((prev) => {
        ...
        if (newTime >= prev.endTime) {
          return { ...prev, currentTime: prev.startTime, isPlaying: false };
        }
  ...
  }, [timeSeriesState, setTimeSeriesState]);

  if (!timeSeriesState) return null;
  ...
  const handleScrub = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTimeSeriesState((prev) =>
      prev ? { ...prev, currentTime: value } : null
    );
  };

  return (
    <div className="absolute bottom-4 left-4 right-4 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-sm rounded-lg border border-zinc-200 dark:border-zinc-800 p-3 flex items-center gap-3">
      <button
        type="button"
        onClick={togglePlay}
        className="px-3 py-1 bg-blue-500 hover:bg-blue-600 text-white rounded text-sm font-medium"
      >
        {timeSeriesState.isPlaying ? "Pause" : "Play"}
      </button>

      <input
        type="range"
        min={timeSeriesState.startTime}
        max={timeSeriesState.endTime}
        step={(timeSeriesState.endTime - timeSeriesState.startTime) / 1000}
        value={timeSeriesState.currentTime}
        onChange={handleScrub}
        className="flex-1"
      />

      <span className="text-sm font-mono text-zinc-700 dark:text-zinc-300">
        {timeSeriesState.currentTime.toFixed(1)}
      </span>
    </div>
  );
}
