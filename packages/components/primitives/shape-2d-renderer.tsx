...
/**
 *
 * ** WebGPU-accelerated 2D shape rendering** with SDF-based anti-aliasing.
...
import { createAlignedUniformData } from "./webgpu-utils";
...
/**
 * Create uniform data for shape renderer
...
export const createPipeline = (
  ...
  shaderModule.getCompilationInfo().then((info: GPUCompilationInfo) => {
    if (info.messages.some((m: GPUCompilationMessage) => m.type === "error")) {
      ...
      info.messages.forEach((msg: GPUCompilationMessage) => {
        if (msg.type === "error") {
...
export const WebGPU2DRenderer: React.FC<WebGPU2DRendererProps> = React.memo(
  ({
    ...
    React.useEffect(() => {
      if (error) {
        console.error("Failed to initialize 2D Shape Renderer:", error);
        onError?.(error);
      }
    }, [error, onError]);
