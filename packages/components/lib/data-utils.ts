 ...
 * Maintains a circular buffer for efficient updates
 */
export class StreamingBuffer {
  ...
  constructor(device: GPUDevice, capacity: number) {
    ...
    this.buffer = device.createBuffer({
      size: capacity * 8, // 2 floats per point (x, y)
      usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST,
    });
  }
   ...
   */
  append(data: DataPoint[]): void {
    const vertexData = dataToVertexArray(data);

    for (let i = 0; i < data.length; i++) {
      const offset = ((this.writePosition + i) % this.capacity) * 8;
    ...
    this.writePosition = (this.writePosition + data.length) % this.capacity;
    this.dataCount = Math.min(this.dataCount + data.length, this.capacity);
  }
