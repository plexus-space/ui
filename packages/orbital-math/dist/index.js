 ...
 */
export function solveKeplerEquation(M, e, tolerance = EPSILON) {
    // Normalize M to [0, 2π]
    M = M % (2 * Math.PI);
    if (M < 0)
        M += 2 * Math.PI;
 ...
 */
export function solveL1Position(mu, tolerance = EPSILON) {
 ...
 */
export function solveL2Position(mu, tolerance = EPSILON) {
    // Initial guess
    let r = 1 + Math.pow(mu / 3, 1 / 3);
    for (let i = 0; i < MAX_ITERATIONS; i++) {
        const f = (1 - mu) / Math.pow(r + mu, 2) + mu / Math.pow(r - 1 + mu, 2) - r;
 ...
 * @returns Distance from barycenter (normalized)
 */
export function solveL3Position(mu, tolerance = EPSILON) {
    // Initial guess
    let r = -1 - 5 * mu / 12;
    for (let i = 0; i < MAX_ITERATIONS; i++) {
        const f = (1 - mu) / Math.pow(r + mu, 2) + mu / Math.pow(r - 1 + mu, 2) + r;
 ...
 */
export function calculateOrbitalPeriod(a, mu = MU_EARTH) {
 ...
 */
export function calculateMeanMotion(a, mu = MU_EARTH) {
 ...
 */
export function calculateOrbitalVelocity(r, a, mu = MU_EARTH) {
 ...
 */
export function calculateEscapeVelocity(r, mu = MU_EARTH) {
 ...
 */
export function propagateMeanAnomaly(M0, n, dt) {
    return (M0 + n * dt) % (2 * Math.PI);
}
export function elementsToStateVector(elements) {
 ...
 */
export function calculateHohmannDeltaV(r1, r2, mu = MU_EARTH) {
