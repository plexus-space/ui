 ...
 */
function orbitalVelocity(radius: number, mu: number): number {
 ...
 * ```
 */
export const TransferOrbitRoot = memo(
  forwardRef<THREE.Group, TransferOrbitRootProps>(
    (
      {
        config,
        segments = DEFAULT_SEGMENTS,
        initialColor = DEFAULT_INITIAL_COLOR,
        finalColor = DEFAULT_FINAL_COLOR,
        transferColor = DEFAULT_TRANSFER_COLOR,
        burnColor = DEFAULT_BURN_COLOR,
        showBurns = false,
        burnSize = 200,
        showLabels = false,
        lineWidth = 2,
      ...
      const transferData = useMemo(() => {
        if (type === "bi-elliptic") {
          ...
          return calculateBiEllipticTransfer(
            initialRadius,
            finalRadius,
            rb,
            mu,
        ...
        <group ref={ref}>
          ...
          <line>
            <bufferGeometry>
              <bufferAttribute
                ...
                count={transferData.initialOrbit.length}
                array={
                  new Float32Array(
                    transferData.initialOrbit.flatMap((p) => [p.x, p.y, p.z])
                  )
                }
                itemSize={3}
              />
            </bufferGeometry>
            <lineBasicMaterial color={initialColor} linewidth={lineWidth} />
          </line>
          ...
          <line>
            <bufferGeometry>
              <bufferAttribute
                attach="attributes-position"
                count={transferData.finalOrbit.length}
                array={
                  new Float32Array(
                    transferData.finalOrbit.flatMap((p) => [p.x, p.y, p.z])
                  )
                }
                itemSize={3}
              />
          ...
          {transferData.transferPaths.map((path, idx) => (
            <line key={`transfer-${idx}`}>
              <bufferGeometry>
                <bufferAttribute
                  attach="attributes-position"
                  count={path.length}
          ...
          {/* Burn markers */}
          {showBurns &&
            transferData.burns.map((burn, idx) => (
              <mesh key={`burn-${idx}`} position={burn.position}>
                <sphereGeometry args={[burnSize, 16, 16]} />
                <meshBasicMaterial color={burnColor} />
              </mesh>
            ))}
        </group>
      );
 ...
 * ```
 */
const TransferOrbitComponent = forwardRef<THREE.Group, TransferOrbitProps>(
  (props, ref) => {
    return (
      <TransferOrbitRoot ref={ref} {...props} />
    );
  }
);
...
export const TransferOrbitUtils = {
   ...
   */
  calculateHohmannDeltaV: (r1: number, r2: number, mu = DEFAULT_MU): number => {
