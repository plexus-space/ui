...
export function useGanttSelection(options = {}) {
    const { initialSelectedId, onSelectionChange, multiSelect = false, } = options;
    // Single selection state
    const [selectedId, setSelectedId] = useState(initialSelectedId || null);
    // Multi-selection state
    const [selectedIds, setSelectedIds] = useState(new Set(initialSelectedId ? [initialSelectedId] : []));
    /**
     * Select a single task
     */
    const select = useCallback((taskId) => {
        if (multiSelect) {
            if (taskId === null) {
