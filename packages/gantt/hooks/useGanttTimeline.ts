...
export function useGanttTimeline(options: UseGanttTimelineOptions) {
   ...
   */
  const getDivisionTimestamp = useMemo(
    () => (index: number) => {
  ...
  const getTaskPosition = useMemo(
    () => (task: GanttTaskData): TaskPosition => {
   ...
   */
  const isTaskVisible = useMemo(
    () => (task: GanttTaskData): boolean => {
      // Task is visible if it overlaps with the time window at all
      return (
        task.endTime >= timeWindow.start && task.startTime <= timeWindow.end
      );
    },
    [timeWindow]
  );
   ...
   */
  const getTaskWidth = useMemo(
    () => (task: GanttTaskData, containerWidth: number): number => {
